...
5. Optional test: Run the Docker container manually on Perlmutter:
   ...
   podman-hpc run --gpu -v /etc/localtime:/etc/localtime -v $HOME/db.profile:/root/db.profile -v /path/to/config.yaml:/app/ml/config.yaml --rm -it registry.nersc.gov/m558/superfacility/ml-training:latest python -u /app/ml/train_model.py --config_file_path /app/ml/config.yaml --model NN
   ```
