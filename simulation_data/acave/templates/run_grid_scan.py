...
if args.single_simulation_parameters is None:
    ...
    var_1 = VaryingParameter(
        "target_to_focus_distance", df["sim_val"].iloc[0], df["sim_val"].iloc[0]
    )
    var_2 = VaryingParameter(
        "fused_silica_thickness", df["sim_val"].iloc[1], df["sim_val"].iloc[1]
    )
...
ev_pre = TemplateEvaluator(
    ...
    sim_files=["templates/retrieval01_spectrum.csv"],
    n_procs=1,
)
...
save_dir = (
    "/global/cfs/cdirs/m558/superfacility/simulation_data/acave/multi_"
