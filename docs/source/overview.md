...
- **Submission script**: The batch script `ml/training_pm.sbatch` is copied into the container image pushed to the NERSC registry and deployed via Spin. It serves as a template for Superfacility API job submission when users launch model training from the GUI.
...
They are copied into the container image pushed to the NERSC registry and deployed via Spin.
